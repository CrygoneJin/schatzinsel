...
| 2026-04-21 | Session 103 (S92-3): Playwright NPC-Dialog Tests implementiert (9 Tests, npc-dialog.spec.js). Lokale Tests scheitern wegen Browser-Version-Mismatch: Playwright 1.58.2 braucht chromium-1208, installiert ist chromium-1194. Download geblockt in Sandbox. CI installiert fresh via `npx playwright install chromium` — dort wird es grün. Gleiches Problem hatten PRs #394/#395. Tests abgedeckt: openChat API (Panel + _lastChatNpcId), Chat-Header, Greeting-Nachricht, NPC-Wechsel, User-Message DOM, ELIZA-Fallback (Proxy geblockt via page.route), INSEL_CHARACTERS Vollständigkeit, ELIZA.transform API. |
