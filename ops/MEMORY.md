...
| 2026-04-22 | Neutrino-Gen-Upgrade-Rezepte (Planck/Beirat, PR #498, Branch `feat/neutrino-recipes`): PR #495-Follow-up. Nach Entfernung der 8 Pauli-Selbst-Upgrade-Regeln (Caves-überall-Fix) fehlten Craft-Rezepte für `neutrino_mu` und `neutrino_tau` — Gen2/Gen3-Neutrinos waren für Oscar unerreichbar. Quark-Ladder + geladene Leptonen hatten Rezepte, Neutrinos nicht. Zwei Zeilen in `recipes.js` hinter Tau-Lepton: `2 neutrino → neutrino_mu` (requires: `['muon']`), `2 neutrino_mu → neutrino_tau` (requires: `['tau']`). Progressive Disclosure analog zu Baryon-Rezepten aus PR #489 — pädagogischer Lernweg: erst Charged-Lepton entdecken, dann entsprechendes Neutrino freischalten. 8 neue Tests in automerge.test.js: Recipe-Existenz + Ingredient-Validität + `requires`-Feld (per-Element-Vergleich statt `deepEqual`, weil vm-Context-Arrays anderen Prototypen haben) + Regression-Guards gegen Re-Introduction der Pauli-Kaskade im Auto-Merge. 95/95 Tests grün, tsc grün. Advisor-Call vor Test-Impl rettete vor `recipeUnlocked()`-Funktions-Test (DOM-Abhängigkeit in game.js) → direkt am Recipe-Objekt testen wie bei Baryon-Recipes. Worktree-Pattern befolgt (`/tmp/s104-neutrinos`). |
